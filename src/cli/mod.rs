...
enum Commands {
    ...
    #[command(about = "Show a read-only workspace merge recommendation")]
    Merge {
        ...
        #[arg(long, help = "Render recommendation as JSON")]
        json: bool,
    },
...
fn try_run(
    ...
    match cli.command {
        ...
        Commands::Merge { from, into, json } => {
            commands::merge::run_merge_preview(&path, &from, into.as_deref(), json)?;
        }
